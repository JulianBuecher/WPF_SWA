...
object KundePatcher {
    ...
    private fun replaceOps(kunde: Kunde, ops: Collection<PatchOperation>): Kunde {
        ...
        ops.forEach { (_, path, value) ->
            when (path) {
                "/nachname" -> kundeUpdated = replaceNachname(kundeUpdated, value)
                "/email" -> kundeUpdated = replaceEmail(kundeUpdated, value)
            }
        }
        return kundeUpdated
    }
    ...
    private fun addInteresse(op: PatchOperation, kunde: Kunde): Kunde {
        ...
        val interesse = InteresseType.build(interesseStr) ?: return kunde
    ...
    private fun removeInteressen(kunde: Kunde, ops: List<PatchOperation>) =
        if (ops.isEmpty()) {
            kunde
        } else {
            var kundeUpdated = kunde
    ...
    private fun removeInteresse(op: PatchOperation, kunde: Kunde): Kunde {
        val interesseStr = op.value
        val interesse = InteresseType.build(interesseStr) ?: kunde
